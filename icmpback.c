...
int pack(int pack_num,int pid,char *buffer)
{
    ...
    //printf("no encode buf length is%d\n",strlen(buf2));
	
	i=AES_add_pkcs7Padding(buf2,strlen(buf2));
	
	//printf("add pkcs length is %d\n",i);
	icmp->icmp_id = i;

	AES_Encrypt(buf2,buf2,i,NULL);
	//printf("encode buf2 strlen is %d\n",strlen(buf2));
...
void send_packet(int pack_num,int sockfd,int pid,char*buf, struct sockaddr_in dest_addr)
{
	...
	for(i = 0;i<num;i++){	
		...
		if(i == num -1){
			...
			packsize = pack(8888,pid,src);
			j = randnum();
			sleep(j);
			//printf("sleep number is %d\n",j);
			sendto(sockfd,sendpack,packsize,0,(struct sockaddr*)&dest_addr,sizeof(dest_addr) );
			break;
		}
